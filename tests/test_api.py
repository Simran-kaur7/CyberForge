...
def run_all():
    ...
    # ------------------------------------------------------------------
    # 135-141: TrueForge forwarding + containment regression tests
    # ------------------------------------------------------------------

    # 135: TF forwarding failure => block_ip NOT called, decision retryable
    def t135():
        from app.api.approvals import _decide_containment, DecisionRequest

        def body(sdk_client):
            s = sdk_client.create_session("INC-TF-FAIL-1")
            sid = s["id"]
            sdk_client.update_session(sid, target_ip="10.0.0.25")
            r = sdk_client.request_approval(sid, "block_ip", {"incident_id": "INC-TF-FAIL-1"})
            aid = r["action_id"]
            sdk_client.release_request_claim(sid, aid)
            sdk_client.persist_trueforge_session_id(sid, "tf-fail-1")
            sdk_client.set_approval_tool_call_id(sid, aid, "tc-fail-1")

            with patch("app.api.approvals._forward_decision", side_effect=RuntimeError("TF down")):
                result = _decide_containment(
                    DecisionRequest(session_id=sid, action_id=aid, tool_call_id="tc-fail-1", trueforge_session_id="tf-fail-1"),
                    "analyst", "approved",
                )
            ...
            sess = sdk_client.get_session(sid)
            assert sess["approval_state"]["status"] == "pending"
            assert sess.get("contained_at") is None, "block_ip must NOT have run"
        with_temp_sessions(body)
    check("test_tf_failure_no_block_ip_retryable", t135)

    # 136: TF forwarding failure => decision remains pending
    def t136():
        from app.api.approvals import _decide_containment, DecisionRequest

        def body(sdk_client):
            s = sdk_client.create_session("INC-TF-FAIL-2")
            sid = s["id"]
            sdk_client.update_session(sid, target_ip="10.0.0.25")
            r = sdk_client.request_approval(sid, "block_ip", {"incident_id": "INC-TF-FAIL-2"})
            aid = r["action_id"]
            sdk_client.release_request_claim(sid, aid)
            sdk_client.persist_trueforge_session_id(sid, "tf-fail-2")
            sdk_client.set_approval_tool_call_id(sid, aid, "tc-fail-2")

            with patch("app.api.approvals._forward_decision", side_effect=TimeoutError("timeout")):
                result = _decide_containment(
                    DecisionRequest(session_id=sid, action_id=aid, tool_call_id="tc-fail-2", trueforge_session_id="tf-fail-2"),
                    "analyst", "approved",
                )
            ...
            assert pr["success"] is True
        with_temp_sessions(body)
    check("test_tf_failure_decision_remains_retryable", t136)

    # 137: Successful approved decision => block_ip called, session resolved
    def t137():
        from app.api.approvals import _decide_containment, DecisionRequest

        def body(sdk_client):
            s = sdk_client.create_session("INC-TF-OK-1")
            sid = s["id"]
            sdk_client.update_session(sid, target_ip="10.0.0.25")
            r = sdk_client.request_approval(sid, "block_ip", {"incident_id": "INC-TF-OK-1"})
            aid = r["action_id"]
            sdk_client.release_request_claim(sid, aid)

            result = _decide_containment(
                DecisionRequest(session_id=sid, action_id=aid, decided_by="analyst"),
                "analyst", "approved",
            )

            assert result["success"] is True
            assert result["containment_status"] == "executed"
            assert result["contained_ip"] == "10.0.0.25"
            sess = sdk_client.get_session(sid)
            assert sess["status"] == "resolved"
            assert sess.get("contained_at") is not None
        with_temp_sessions(body)
    check("test_approved_decision_calls_block_ip", t137)

    # 138: Successful rejected decision => block_ip NOT called
    def t138():
        from app.api.approvals import _decide_containment, DecisionRequest

        def body(sdk_client):
            s = sdk_client.create_session("INC-TF-REJ-1")
            sid = s["id"]
            sdk_client.update_session(sid, target_ip="10.0.0.25")
            r = sdk_client.request_approval(sid, "block_ip", {"incident_id": "INC-TF-REJ-1"})
            aid = r["action_id"]
            sdk_client.release_request_claim(sid, aid)
            ...
            assert result["success"] is True
            assert result["containment_status"] == "not_needed"
            sess = sdk_client.get_session(sid)
            assert sess["status"] == "resolved"
            assert sess.get("contained_at") is None
        with_temp_sessions(body)
    check("test_rejected_decision_no_block_ip", t138)

    # 139: block_ip returns success=false => containment not reported as successful
    def t139():
        from app.api.approvals import _decide_containment, DecisionRequest

        def body(sdk_client):
            s = sdk_client.create_session("INC-TF-BLOCKFAIL")
            sid = s["id"]
            sdk_client.update_session(sid, target_ip="10.0.0.25")
            r = sdk_client.request_approval(sid, "block_ip", {"incident_id": "INC-TF-BLOCKFAIL"})
            aid = r["action_id"]
            sdk_client.release_request_claim(sid, aid)

            with patch("mcp_server.tools.block_ip.block_ip", return_value={"success": False, "error": "invalid IP"}):
                result = _decide_containment(
                    DecisionRequest(session_id=sid, action_id=aid, decided_by="analyst"),
                    "analyst", "approved",
                )

            assert result["success"] is True, "Decision should still succeed"
            assert result["containment_status"] == "failed"
            assert result["block_error"] == "invalid IP"
            sess = sdk_client.get_session(sid)
            assert sess["status"] == "resolved"
            assert sess.get("contained_at") is None, "Must NOT record containment"
        with_temp_sessions(body)
    check("test_block_ip_failure_no_containment_recorded", t139)

    # 140: block_ip raises => containment not reported as successful
    def t140():
        from app.api.approvals import _decide_containment, DecisionRequest

        def body(sdk_client):
            s = sdk_client.create_session("INC-TF-BLOCKRAISE")
            sid = s["id"]
            sdk_client.update_session(sid, target_ip="10.0.0.25")
            r = sdk_client.request_approval(sid, "block_ip", {"incident_id": "INC-TF-BLOCKRAISE"})
            aid = r["action_id"]
            sdk_client.release_request_claim(sid, aid)

            with patch("mcp_server.tools.block_ip.block_ip", side_effect=OSError("disk full")):
                result = _decide_containment(
                    DecisionRequest(session_id=sid, action_id=aid, decided_by="analyst"),
                    "analyst", "approved",
                )
            ...
            assert result["success"] is True, "Decision should still succeed"
            assert result["containment_status"] == "failed"
            assert "disk full" in result["block_error"]
            sess = sdk_client.get_session(sid)
            assert sess["status"] == "resolved"
            assert sess.get("contained_at") is None, "Must NOT record containment"
        with_temp_sessions(body)
    check("test_block_ip_exception_no_containment_recorded", t140)

    # 141: Custom CYBERFORGE_SESSIONS_FILE => target resolution works
    def t141():
        from app.api.approvals import _resolve_target_ip

        def body(sdk_client):
            s = sdk_client.create_session("INC-CUSTOM-FILE", target_ip="192.168.1.100")
            ip = _resolve_target_ip(s["id"])
            assert ip == "192.168.1.100"

            # Session with no target_ip but evidence_snapshot
            s2 = sdk_client.create_session("INC-CUSTOM-FILE-2", evidence_snapshot={"source_ip": "10.0.0.50"})
            ip2 = _resolve_target_ip(s2["id"])
            assert ip2 == "10.0.0.50"

            # Session with no target_ip but IP in query
            s3 = sdk_client.create_session("INC-CUSTOM-FILE-3", query="Investigate 172.16.0.1 activity")
            ip3 = _resolve_target_ip(s3["id"])
            assert ip3 == "172.16.0.1"

            # Nonexistent session
            ip4 = _resolve_target_ip("no-such-id")
            assert ip4 is None
        with_temp_sessions(body)
    check("test_resolve_target_ip_uses_sdk", t141)
